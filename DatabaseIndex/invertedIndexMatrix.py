...
def update_index_matrix(record: dict[str, str], memory_location: str | Path):
    """
...
if __name__ == "__main__":
    ...
    path = '../MockData/PNR Records/'
    files = get_contents(path)
    for file in files:
        record = read_record(file)
        record = flatten_and_filter_dictionary(record)
        update_index_matrix(record, file)

    with open(f'InvertedIndexMatrix.json', 'w') as fp:
