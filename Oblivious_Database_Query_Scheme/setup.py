...
from Oblivious_Database_Query_Scheme.getters import get_sort_and_encrypt_circuit_path as sort_and_encrypt_circuit_path
from Oblivious_Database_Query_Scheme.getters import get_sort_and_reencrypt_circuit_path as sort_and_reencrypt_circuit_path
from Oblivious_Database_Query_Scheme.getters import get_aes_128_with_circuit_mpc_script_path as aes_128_ecb_with_circuit_mpc_script_path
...
def setup_MPC_scripts():
    ...
    run(["cp", f"{sort_and_encrypt_circuit_path()}", f"{MP_SPDZ_circuits_directory() / sort_and_encrypt_circuit_path().name}"])
    run(["cp", f"{sort_and_reencrypt_circuit_path()}", f"{MP_SPDZ_circuits_directory() / sort_and_reencrypt_circuit_path().name}"])

    run(["cp", f"{sort_and_encrypt_with_circuit_mpc_script_path()}", f"{MP_SDPZ_scripts_directory() / sort_and_encrypt_with_circuit_mpc_script_path().name}"])
    run([f"{MP_SPDZ_compile_path()}", f"{sort_and_encrypt_with_circuit_mpc_script_path().name}", "-F", "128"])
    run(["cp", f"{sort_and_reencrypt_with_circuit_mpc_script_path()}", f"{MP_SDPZ_scripts_directory() / sort_and_reencrypt_with_circuit_mpc_script_path().name}"])
    run([f"{MP_SPDZ_compile_path()}", f"{sort_and_reencrypt_with_circuit_mpc_script_path().name}", "-F", "128"])
    run(["cp", f"{aes_128_ecb_with_circuit_mpc_script_path()}", f"{MP_SDPZ_scripts_directory() / aes_128_ecb_with_circuit_mpc_script_path().name}"])
