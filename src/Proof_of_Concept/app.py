...
def update_mpc_script():
    ...
    with open('../MP_SPDZ_Scripts/proof_of_concept.mpc', 'r') as f:
        script = f.read()
...
def compile_and_run_MP_SPDZ():
    ...
    run(['cp', '../MP_SPDZ_Scripts/proof_of_concept.mpc', '../../../../MP-SPDZ/Programs/Source/proof_of_concept.mpc'])
    chdir('../../../../MP-SPDZ/')
    run(['./compile.py', 'MP_SPDZ_Only_Scheme'])
    client = Popen(['./atlas-party.x', 'MP_SPDZ_Only_Scheme', '-p', '0', '-N', '3', '-IF',
         '../PycharmProjects/MasterThesis/src/Proof_of_Concept/Client/MP_SPDZ_Inputs/MP_SPDZ_Only_Input', '-OF',
         '../PycharmProjects/MasterThesis/src/Proof_of_Concept/Client/MP_SPDZ_Outputs/MP_SPDZ_Only_Output'
         ], stdout=PIPE, stderr=PIPE)
    server = Popen(['./atlas-party.x', 'MP_SPDZ_Only_Scheme', '-p', '1', '-N', '3', '-IF',
         '../PycharmProjects/MasterThesis/src/Proof_of_Concept/Server/MP_SPDZ_Inputs/MP_SPDZ_Only_Input'
         ], stdout=PIPE, stderr=PIPE)
    ...
    chdir('../PycharmProjects/MasterThesis/src/Proof_of_Concept/')
